...
document.getElementById('cs07-heightInput').addEventListener('input', getHeightInput);

function getHeightInput(e) {
    document.getElementById('cs07-output').style.visibility = 'visible';
    document.getElementById('cs07-resetButton').style.visibility = 'visible';
    height = e.target.value;
    document.getElementById('cs07-heightOutput').innerHTML = height; //placing the value to the DOM
    heightConverter();
}

document.getElementById('cs07-firOpt').addEventListener('change', getfirOpt);

function getfirOpt(e) {
    firOpt = e.target.value; //Gets placeholder value
    document.getElementById('cs07-heightInput').placeholder = 'Enter ' + firOpt + '...';
    document.getElementById('cs07-heightName').innerHTML = firOpt + ":";
    heightConverter();
}

document.getElementById('cs07-secOpt').addEventListener('change', getsecOpt);
...
function heightConverter() {
    const finalOutput = document.querySelector('#cs07-finalOutput');
    //Validates for Miles
...
document.getElementById('cs07-resetButton').addEventListener('click', reset);

function reset() {
    document.getElementById('cs07-mainForm').reset();
    document.getElementById('cs07-output').style.visibility = 'hidden';
    document.getElementById('cs07-resetButton').style.visibility = 'hidden';
}
